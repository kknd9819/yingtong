...
(function($) {
	...
	$.message = function() {
		...
		if ($.isPlainObject(arguments[0])) {
			message = arguments[0];
			if (arguments.length > 1) {
				message.callback = arguments[1];
			}
		} else if (typeof arguments[0] === "string" && typeof arguments[1] === "string") {
			message.type = arguments[0];
			message.content = arguments[1];
			if (arguments.length > 2) {
				message.callback = arguments[2];
			}
		} else {
			return false;
		}
		...
		if ($message == null) {
			$message = $('<div class="xxMessage"><div class="delete"><a href="javascript:;" title="关闭">关闭</a></div><div class="messageContent message' + message.type + 'Icon"><\/div><\/div>');
			if (!window.XMLHttpRequest) {
				$message.append('<iframe class="messageIframe"><\/iframe>');
			}
			$message.appendTo("body");
			$message.children(".delete").click(function() {
				$message.hide();
			});
		} else {
			$message.css({
		...
		$message.children(".delete").show();
		$message.children(".messageContent").removeClass("messagewarnIcon messageerrorIcon messagesuccessIcon").addClass("message" + message.type + "Icon").html(message.content);
		if (message.type == "success") {
			...
			$message.css({
				...
				"margin-right" : parseInt($message.outerWidth() / 2),
				"z-index" : zIndex++
			}).show();
		}
		return $message;
	...
	$.dialog = function(options) {
		...
		if ($(settings.content).find('div.error').length > 0) {
			settings.onOk = null;
		}
	...
	$.fn.extend({
		browser : function(options) {
			var settings = {
				...
				uploadUrl : shopxx.base + "/admin/file/upload.jhtml?urlType=0",
				callback : null
			};
			$.extend(settings, options);
					...
					function createBrowserList(path, data) {
						...
						$.each(data, function(i, fileInfo) {
							...
							if (fileInfo.isDirectory) {
								iconUrl = shopxx.base + "/resources/images/folder_icon.gif";
								title = fileInfo.name;
								...
								iconUrl = shopxx.base + "/resources/images/file_icon.gif";
								title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
							}
							// mod 20140721 后台上传图片时从程序中选择图片时路径取相对路径
							var uploadUrl = fileInfo.url;
							var index = uploadUrl.indexOf("/upload");
							if (index > 0) {
								uploadUrl = uploadUrl.substring(index, uploadUrl.length);
							}
							// end
							browserListHtml += '<div class="browserItem"><img src="' + iconUrl + '" title="' + title + '" url="' + uploadUrl + '" isDirectory="' + fileInfo.isDirectory + '" \/><div>' + fileInfo.name + '<\/div><\/div>';
						});
						...
						$browserList.find("img").bind("click", function() {
							...
							if (isDirectory == "true") {
								...
								if (settings.input != null) {
									...
									if ($browserButton.parent().find("a") != null && $browserButton.parent().find("a") != "undefined") {
										$browserButton.parent().find("a").attr("href", $("#siteUrl").val() + url);
									}
								}
						...
						if ($.trim(text) != "") {
							...
							if (data.message.type == "success") {
								if (settings.input != null) {
									...
									if ($browserButton.parent().find("a") != null && $browserButton.parent().find("a") != "undefined") {
										$browserButton.parent().find("a").attr("href", $("#siteUrl").val() + data.url);
									}
								}
								if (settings.callback != null && typeof settings.callback == "function") {
