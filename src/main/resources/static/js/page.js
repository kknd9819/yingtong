...
function pageInit(elementId,currentPage,pageCount,methodName){
	    	...
	    	strPage += getJumpPage(currsPage,totalPage,methodName);
...
/*
		...
		function getPrevAndFistPage(currsPage,methodName){
			...
			if(currsPage == 1){  
        		...
        		strPrevAdnFist += "<a class='firstPage' href='javascript:void(0);' onclick='"+methodName+"(1)'></a>"; 
        		strPrevAdnFist += "<a class='previousPage' href='javascript:void(0);' onclick='"+methodName+"("+(currsPage-1)+")'></a>";
	        	strPrevAdnFist += "<a  href='javascript:void(0);' onclick='"+methodName+"(1)'>1</a>"; 
	        } 
		...
		function getMiddlePage(currsPage,grountCount,currsPageYMin,totalPage,currsPageYMax,methodName){
			...
			for(var i = 0;i<grountCount;i++){
	            ...
	            if(middlePage == currsPage){  
	            	...
	            	strMidlePage += " <a href='javascript:void(0);' onclick='"+methodName+"("+middlePage+")'>"+middlePage+"</a>";
		...
		function getLastPage(currsPage,totalPage,methodName){
			...
			if(currsPage == totalPage){  
	        	...
	        	strLastPage += "<a href='javascript:void(0);' onclick='"+methodName+"("+totalPage+")'>"+totalPage+"</a>";
	        } 
			return strLastPage;
		}
		...
		function getNextPage(currsPage,totalPage,methodName){
			...
			if(currsPage == totalPage){  
        		...
        		strNextPage += "<a class='nextPage' href='javascript:void(0);' onclick='"+methodName+"("+(currsPage+1)+")'></a>";
        		strNextPage += "<a class='lastPage' href='javascript:void(0);' onclick='"+methodName+"("+totalPage+")'></a>";
	        } 
			return strNextPage;
		}
		...
		function getJumpPage(currsPage,totalPage,methodName){
			...
			jumpPageStr += "<font onclick='jumpPage("+totalPage+",\""+methodName+"\")' style='border: 1px solid #cccccc;padding: 2px 5px;cursor:pointer;'>跳转</font>";
			jumpPageStr += "</span>";
			return jumpPageStr;
		}
		
		function jumpPage(totalPage,methodName){
